...
function getWorkspaceId(env: CmuxEnv = process.env): string | undefined {
  return nonEmpty(env.CMUX_WORKSPACE_ID) ?? nonEmpty(env.CMUX_TAB_ID);
}

function getSurfaceId(env: CmuxEnv = process.env): string | undefined {
...
async function getTmuxCmuxEnv(env: CmuxEnv = process.env, runner: CommandRunner = execFileRunner): Promise<CmuxEnv> {
...
async function resolveRuntimeCmuxEnv(env: CmuxEnv = process.env, runner: CommandRunner = execFileRunner): Promise<CmuxEnv> {
...
function pickBestSurfaceId(surfaceList: unknown): string | undefined {
...
function parseSurfaceListOutput(output: string): string | undefined {
  return pickBestSurfaceId(JSON.parse(output));
}

function buildSurfaceListArgs(workspaceId: string): string[] {
  return ["rpc", "surface.list", JSON.stringify({ workspace_id: workspaceId })];
}

function normalizeLogLevel(level: PiCmuxLogLevel | undefined): string | undefined {
...
const execFileRunner: CommandRunner = (command, args, options) =>
  new Promise((resolve) => {
    execFile(
      command,
      [...args],
...
async function resolveCmuxSurfaceId(
  env: CmuxEnv = process.env,
