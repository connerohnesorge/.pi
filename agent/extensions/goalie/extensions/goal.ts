...
export default function goalExtension(pi: ExtensionAPI): void {
	...
	// /goalie: read-only status or new goalie topic.
	pi.registerCommand("goalie", {
		description: "Set or show the session-scoped Stop hook condition (goalie). E.g. /goalie <description> to discuss/draft a stop hook, or /goalie with no arguments to see status.",
	...
	pi.registerCommand("goalie-status", {
	...
	pi.registerCommand("goalie-list", {
		description: "List all open goalies and show which one is focused.",
		handler: async (_rawArgs, ctx) => {
			reconcileFocusedGoalFromDisk(ctx);
			ctx.ui.notify(buildGoalListText(goalsById, focusedGoalId), "info");
			updateUI(ctx);
	...
	pi.registerCommand("goalie-focus", {
		description: "Choose which open goalie this session should focus on.",
		handler: async (_rawArgs, ctx) => {
			await focusGoalCommand(ctx);
		},
	});
	pi.registerCommand("goalie-settings", {
		description: "Open goalie settings.",
		handler: async (_rawArgs, ctx) => {
			await handleGoalSettings(ctx);
		},
	});
	pi.registerCommand("goalie-set", {
		description: "Immediately set the goalie stop hook condition from the supplied natural language contract. No draft discussion.",
		handler: async (rawArgs, ctx) => {
	...
	pi.registerCommand("goalie-edit", {
		...
		handler: async (_rawArgs, ctx) => {
			reconcileFocusedGoalFromDisk(ctx);
			if (!state.goal) {
				ctx.ui.notify("No goalie is currently set/active. Use /goalie-set to set one.", "warning");
				return;
			}
			let activePath = state.goal.activePath;
			if (!activePath) {
				// Write the goalie file first if it doesn't exist on disk
				state.goal = writeActiveGoalFile(ctx, state.goal);
				activePath = state.goal.activePath;
			}
			if (!activePath) {
				ctx.ui.notify("No file on disk for the current goalie.", "warning");
				return;
			}
			const absolutePath = path.resolve(ctx.cwd, activePath);
			const editor = process.env.VISUAL || process.env.EDITOR || "vi";
			try {
				ctx.ui.notify(`Opening ${activePath} in ${editor}...`, "info");
				spawnSync(editor, [absolutePath], { stdio: "inherit" });
				
				// Re-read from disk to pick up edits
				reconcileFocusedGoalFromDisk(ctx);
				ctx.ui.notify("Goalie file reloaded from disk.", "info");
				updateUI(ctx);
			} catch (err) {
				ctx.ui.notify(`Failed to launch editor: ${err instanceof Error ? err.message : String(err)}`, "error");
			}
		},
	});
	pi.registerCommand("goalie-clear", {
		description: "Archive the current goalie.",
		handler: async (_rawArgs, ctx) => {
			await handleGoalClear(ctx);
		},
	});
	pi.registerCommand("goalie-abort", {
		description: "Abort the current goalie, or cancel drafting.",
		handler: async (_rawArgs, ctx) => {
			await handleGoalAbort(ctx);
		},
	});
	pi.registerCommand("goalie-pause", {
		description: "Pause the currently running goalie.",
		handler: async (_rawArgs, ctx) => {
			await handleGoalPause(ctx);
		},
	});
	pi.registerCommand("goalie-resume", {
		description: "Resume a paused goalie.",
		handler: async (_rawArgs, ctx) => {
