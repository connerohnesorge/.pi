	...
	const parallelResults = await mapConcurrent(
		...
		async (task, taskIndex) => {
			...
			if (input.foregroundControl) {
				markControlStart(input.foregroundControl, task.agent, input.globalTaskIndex + taskIndex);
	...
	for (let stepIndex = 0; stepIndex < chainSteps.length; stepIndex++) {
		...
		if (isParallelStep(step)) {
			...
			if (foregroundControl) {
				markControlStart(foregroundControl, seqStep.agent, globalTaskIndex);
				...
				onUpdate: onUpdate
					? (p) => {
						...
						if (foregroundControl && stepProgress.length > 0) {
							...
							applyProgressToForegroundControl(foregroundControl, stepProgress[0]);
						}
						onUpdate({
							...p,
							details: {
