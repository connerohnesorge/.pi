...
function foregroundStatusResult(control: ForegroundControl): AgentToolResult<Details> {
...
function runAsyncPath(data: ExecutionContextData, deps: ExecutorDeps): AgentToolResult<Details> | null {
	...
	if (hasTasks && params.tasks) {
		...
		const outputOverrides = params.tasks.map((task, index) => normalizeOverrideInput(task, agentConfigs[index]?.output).output);
		const parallelTasks = params.tasks.map((task, index) => ({
			...
			...(outputOverrides[index] !== undefined ? { output: outputOverrides[index] } : {}),
...
async function runForegroundParallelTasks(input: ForegroundParallelRunInput): Promise<SingleResult[]> {
	return mapConcurrent(input.tasks, input.concurrencyLimit, async (task, index) => {
		...
		if (input.foregroundControl) {
			markControlStart(input.foregroundControl, task.agent, index);
			wireForegroundInterrupt(input.foregroundControl, interruptController);
		}
		...
		return runSync(input.ctx.cwd, input.agents, task.agent, taskText, {
				...
				onUpdate: input.onUpdate
					? (progressUpdate) => {
						...
						if (input.foregroundControl && stepProgress.length > 0) {
							...
							applyProgressToForegroundControl(input.foregroundControl, stepProgress[0]);
						}
			...
			if (input.foregroundControl?.currentIndex === index) {
				clearControlInterrupt(input.foregroundControl);
...
async function runParallelPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	const outputOverrides: (string | false | undefined)[] = tasks.map((task, index) =>
		normalizeOverrideInput(task, agentConfigs[index]?.output).output,
	);
...
async function runSinglePath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	if (foregroundControl) {
		markControlStart(foregroundControl, params.agent!, 0);
		wireForegroundInterrupt(foregroundControl, interruptController);
	}
	...
	const forwardSingleUpdate = onUpdate
		? (update: AgentToolResult<Details>) => {
			if (foregroundControl) {
				...
				applyProgressToForegroundControl(foregroundControl, firstProgress);
			}
			onUpdate(update);
		}
		: undefined;
	...
	if (foregroundControl?.currentIndex === 0) {
		applyProgressToForegroundControl(foregroundControl, r.progress);
		clearControlInterrupt(foregroundControl);
	}
