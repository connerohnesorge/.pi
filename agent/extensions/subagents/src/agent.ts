...
import { buildDefaultTierConfig, loadModelTierConfig, type ModelTierConfig, resolveTierModel } from "./model-tier-config.ts";
...
export function resolveAgentModelSpec(
  ...
  const config = loadConfig() ?? buildDefaultTierConfig(mainModel);
  if (options.tier) return resolveTierModel(options.tier, config) ?? mainModel;
  return resolveTierModel("medium", config) ?? mainModel;
}
...
export class WorkflowAgent {
  ...
  private resolveModel(spec: string, perRunRegistry?: ModelRegistry): Model<any> | undefined {
    ...
    const isUsable = (model: Model<any> | undefined) => {
      if (!model) return false;
      const check = (registry as { hasConfiguredAuth?: (model: Model<any>) => boolean }).hasConfiguredAuth;
      return check ? check.call(registry, model) : true;
    };
    const slash = spec.indexOf("/");
    if (slash > 0) {
      const model = registry.find(spec.slice(0, slash), spec.slice(slash + 1));
      return isUsable(model) ? model : undefined;
    }
    return registry.getAvailable().find((m) => m.id === spec) ?? registry.getAll().find((m) => m.id === spec && isUsable(m));
  }
  ...
  private resolveModelAndThinking(
    ...
    const exact = this.resolveModel(spec, perRunRegistry);
    if (exact) return { model: exact };

    const colon = spec.lastIndexOf(":");
    if (colon <= 0) return {};
    const suffix = spec.slice(colon + 1) as ThinkingLevel;
    if (!THINKING_LEVELS.has(suffix)) return {};
    const model = this.resolveModel(spec.slice(0, colon), perRunRegistry);
