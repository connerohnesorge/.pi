...
import { formatNestedAggregate, walkNestedRuns } from "../runs/shared/nested-render.ts";
import { aggregateStepStatus, formatActivityLabel, formatAgentRunningLabel, formatParallelOutcome } from "../shared/status-format.ts";
import { buildChainStepSpansFromGroups } from "./chain-spans.ts";
import { deriveForegroundRunModel } from "./render-model.ts";
...
function widgetChainDetails(job: AsyncJobState, theme: Theme, expanded = false, width = getTermWidth()): string[] {
	...
	const lines: string[] = [];
	for (const span of buildChainStepSpansFromGroups(total, job.steps.length, job.parallelGroups)) {
...
function formatNestedWidgetLines(children: NestedRunSummary[] | undefined, theme: Theme, width: number, expanded: boolean, snapshotNow?: number, lineBudget = expanded ? 12 : 1): string[] {
	...
	let stepRunSnapshot = snapshotNow;
	return walkNestedRuns(children, {
		maxDepth: 2,
		maxLines: lineBudget,
		renderRun: (child, _depth, prefix) => {
			stepRunSnapshot = snapshotNow ?? child.lastUpdate;
			const activity = nestedActivity(child, child.state, snapshotNow ?? child.lastUpdate);
			const error = child.error ? ` · ${child.error}` : "";
			return theme.fg("dim", `${prefix}↳ ${nestedStatusGlyph(child.state, theme, nestedRunSeed(child))} ${nestedRunName(child)} · ${child.state} · ${activity}${error}`);
		},
		renderStep: (step, _stepIndex, prefix) => theme.fg("dim", `${prefix}↳ ${nestedStatusGlyph(step.status, theme)} ${step.agent} · ${step.status} · ${nestedActivity(step, step.status, stepRunSnapshot)}`),
		renderAggregate: (text, prefix) => theme.fg("dim", `${prefix}↳ ${text}`),
		postProcess: (lines) => lines.map((line) => truncLine(line, width)),
	});
}
...
function renderMultiCompact(d: Details, theme: Theme): Component {
	...
	const model = deriveForegroundRunModel(d, hasRunning);
	const itemTitle = model.itemTitle;
	const stats = statJoin(theme, [model.header, formatProgressStats(theme, totalSummary)]);
	...
	const useResultsDirectly = model.hasParallelInChain || !d.chainAgents?.length;
	const displayStart = model.activeGroup ? model.activeGroup.start : 0;
	const displayEnd = model.activeGroup ? model.activeGroup.end : (useResultsDirectly ? d.results.length : d.chainAgents!.length);
	for (let i = displayStart; i < displayEnd; i++) {
		const r = d.results[i];
		const fallbackLabel = itemTitle.toLowerCase();
		const rowNumber = model.activeGroup ? (i - model.activeGroup.start + 1) : (i + 1);
		...
		const stepLabel = model.rows[i]!.label;
...
export function renderSubagentResult(
	...
	const chainVis = d.chainAgents?.length && !model.hasParallelInChain
		? d.chainAgents
				.map((agent, i) => {
	...
	c.addChild(
		new Text(
			fit(`${icon} ${theme.fg("toolTitle", theme.bold(modeLabel))}${contextBadge} · ${model.header}${summaryStr}`),
			0,
			0,
		),
	);
