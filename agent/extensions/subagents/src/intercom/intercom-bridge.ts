...
/** @internal Structured result of {@link evaluateIntercomGate}; exported for unit tests only. */
export interface IntercomGateDecision {
	active: boolean;
	reason?: string;
	mode: IntercomBridgeMode;
	extensionDir: string;
	...
	wantsIntercom: boolean;
	piIntercomAvailable: boolean;
	configStatus?: { enabled: boolean; error?: unknown };
}
...
/**
 * Single owner of the intercom activation gate ladder. Both {@link resolveIntercomBridge}
 * (runtime) and {@link diagnoseIntercomBridge} (doctor) project their `active` outcome from
 * this one evaluator instead of re-walking the gates independently.
 *
 * @internal Exported only so the gate ladder can be unit-tested directly; not part of the
 * public surface consumed by the runtime/doctor callers.
 */
export function evaluateIntercomGate(input: ResolveIntercomBridgeInput): IntercomGateDecision {
	...
	let configStatus: { enabled: boolean; error?: unknown } | undefined;
...
export function diagnoseIntercomBridge(input: ResolveIntercomBridgeInput): IntercomBridgeDiagnostic {
	const gate = evaluateIntercomGate(input);
	let intercomConfigError: string | undefined;
	if (gate.configStatus?.error) {
		const error = gate.configStatus.error;
	...
	return {
		active: gate.active,
		mode: gate.mode,
		wantsIntercom: gate.wantsIntercom,
		piIntercomAvailable: gate.piIntercomAvailable,
		extensionDir: gate.extensionDir,
		configPath: gate.configPath,
		...(gate.orchestratorTarget ? { orchestratorTarget: gate.orchestratorTarget } : {}),
		...(gate.reason ? { reason: gate.reason } : {}),
		...(gate.configStatus ? { intercomConfigEnabled: gate.configStatus.enabled } : {}),
...
export function resolveIntercomBridge(input: ResolveIntercomBridgeInput): IntercomBridgeState {
	const config = resolveIntercomBridgeConfig(input.config);
	const gate = evaluateIntercomGate(input);
	const { mode, extensionDir, orchestratorTarget } = gate;
	...
	if (gate.configStatus?.error) console.warn(`Failed to parse intercom config at '${gate.configPath}'. Assuming enabled.`, gate.configStatus.error);
	if (!gate.active) {
