...
// Border characters for the overlay box. Double-line on purpose: workflows should stand out.
...
export function openWorkflowNavigator(
  ...
  return ui.custom<void>(
    (tui: TUI, theme: Theme, _keybindings, done: (r: undefined) => void) => {
      ...
      // Wrap the rendered content inside a prominent visual box border.
      let _focused = false;
      const component: Component & Focusable & { dispose?(): void } = {
        ...
        render: (width: number) => {
          ...
          const topBorder =
            borderColor("╔═") + title + borderColor("═".repeat(Math.max(0, innerWidth + 2 - visibleWidth(" WORKFLOWS ") - 1))) + borderColor("╗");
