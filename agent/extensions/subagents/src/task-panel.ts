...
function borderedPanel(title: string, body: string[], theme: Theme, width?: number): string[] {
  if (!body.length) return [];
  const natural = Math.max(
    visibleWidth(title) + 6,
    ...body.map((line) => visibleWidth(line) + 4),
  );
  const totalWidth = Math.max(16, Math.floor(width ?? natural));
  const innerWidth = Math.max(1, totalWidth - 4);
  const border = (s: string) => theme.fg("borderAccent", s);
  const titleText = ` ${title} `;
  const titleRoom = Math.max(0, innerWidth - 1);
  const clippedTitle = truncateToWidth(titleText, titleRoom, "", false);
  const top = border("╔═") + theme.fg("accent", theme.bold(clippedTitle)) + border("═".repeat(Math.max(0, innerWidth - visibleWidth(clippedTitle)))) + border("═╗");
  ...
  return [top, ...rows, bottom].map((line) => fitLine(line, width));
}
...
export function renderPanel(manager: WorkflowManager, theme: Theme, width?: number): string[] {
  ...
  return borderedPanel(`Workflows running (${active.length})`, [...rows, hint], theme, width);
}
