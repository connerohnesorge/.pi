...
function withEnabledIntercom<T>(fn: (paths: { extensionDir: string; configPath: string }) => T): T {
	const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-intercom-gate-test-"));
...
describe("evaluateIntercomGate", () => {
	it("gate 1: reports inactive when bridge mode is off", () => {
		const decision = evaluateIntercomGate({
			config: { mode: "off" },
			context: "fork",
		...
		assert.equal(decision.reason, "bridge mode is off");
		assert.equal(decision.configStatus, undefined);
	});

	it("gate 2: reports inactive when fork-only and context is not fork", () => {
		const decision = evaluateIntercomGate({
		...
		assert.equal(decision.active, false);
		assert.equal(decision.reason, "bridge mode is fork-only and context is not fork");
		assert.equal(decision.wantsIntercom, false);
	});

	it("gate 3: reports inactive when orchestrator target is missing", () => {
		const decision = evaluateIntercomGate({
			config: { mode: "always" },
			context: "fresh",
			orchestratorTarget: "   ",
		...
		assert.equal(decision.active, false);
		assert.equal(decision.reason, "orchestrator target is not available");
		assert.equal(decision.orchestratorTarget, "");
	});

	it("gate 4: reports inactive when the pi-intercom extension is missing", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-intercom-gate-test-"));
		try {
			const decision = evaluateIntercomGate({
				config: { mode: "always" },
			...
			assert.equal(decision.active, false);
			assert.equal(decision.reason, "pi-intercom extension was not found");
			assert.equal(decision.piIntercomAvailable, false);
			assert.equal(decision.configStatus, undefined);
		} finally {
			fs.rmSync(tempDir, { recursive: true, force: true });
		}
	...
	it("gate 5: reports inactive when intercom config is disabled", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-intercom-gate-test-"));
		const extensionDir = path.join(tempDir, "pi-intercom");
		const configPath = path.join(tempDir, "config.json");
		fs.mkdirSync(extensionDir, { recursive: true });
		...
		try {
			...
			assert.equal(decision.active, false);
			assert.equal(decision.reason, "intercom config is disabled");
			assert.equal(decision.configStatus?.enabled, false);
		} finally {
			fs.rmSync(tempDir, { recursive: true, force: true });
		}
	});

	it("active: reports active when every gate passes", () => {
		withEnabledIntercom(({ extensionDir, configPath }) => {
			const decision = evaluateIntercomGate({
				config: { mode: "always" },
				context: "fresh",
				orchestratorTarget: "  main  ",
			...
			assert.equal(decision.orchestratorTarget, "main");
			assert.equal(decision.piIntercomAvailable, true);
			assert.equal(decision.configStatus?.enabled, true);
		});
	});
});
