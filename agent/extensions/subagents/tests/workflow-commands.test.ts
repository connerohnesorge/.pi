...
import { createEffortState, effortDirective } from "../src/effort-command.ts";
import { registerWorkflowCommands } from "../src/workflow-commands.ts";
import { buildForcedWorkflowPrompt, WORKFLOW_TOOL_NAME } from "../src/workflow-editor.ts";
import type { WorkflowManager } from "../src/workflow-manager.ts";
...
test("/workflows save <name> <runId> saves the specified run", async () => {
  ...
  const h = harness({ listRuns: () => runs }, { storage: { save: (w: any) => (saved.push(w), { ...w, id: "saved-2" }) } });
  ...
  assert.ok(h.notified.some((n) => n.message.includes("Saved")), "should notify Saved");
});
