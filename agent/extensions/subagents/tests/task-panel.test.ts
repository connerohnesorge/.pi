...
before(async () => {
  mod = (await import("../src/task-panel.ts")) as TaskPanelModule;
});
...
describe("renderPanel", () => {
  ...
  it("hints that finished runs are kept in /workflows history", async () => {
    const { renderPanel } = await import("../src/task-panel.ts");
    const manager = {
      listRuns: () => [
...
describe("token rate", () => {
  it("returns 0 with fewer than two samples and after clearing", async () => {
    const { sampleTokens, tokensPerSecond, clearTokenSamples } = await import("../src/task-panel.ts");
...
describe("renderPanelDetailed", () => {
  ...
  it("renders aggregate tokens, cost, phases, and per-agent rows", async () => {
    const { renderPanelDetailed, clearTokenSamples } = await import("../src/task-panel.ts");
