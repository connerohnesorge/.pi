...
import type { ModelTierConfig } from "../src/model-tier-config.ts";
...
test("resolveAgentModelSpec: no saved config falls back to GPT-5.5 tier defaults", () => {
  assert.equal(resolveAgentModelSpec({ tier: "small" }, "main/model", noCfg), "openai-codex/gpt-5.5:low");
  assert.equal(resolveAgentModelSpec({ tier: "medium" }, "main/model", noCfg), "openai-codex/gpt-5.5:medium");
  assert.equal(resolveAgentModelSpec({ tier: "big" }, "main/model", noCfg), "openai-codex/gpt-5.5:high");
...
test("resolveAgentModelSpec: untagged agent with NO config defaults to GPT-5.5 medium", () => {
  assert.equal(resolveAgentModelSpec({}, "main/model", noCfg), "openai-codex/gpt-5.5:medium");
});
...
test("resolveAgentModelSpec: tier with no main model and no config still uses GPT-5.5 defaults", () => {
  assert.equal(resolveAgentModelSpec({ tier: "small" }, undefined, noCfg), "openai-codex/gpt-5.5:low");
});
...
test("WorkflowAgent.resolveModelAndThinking splits Pi thinking suffixes", () => {
  const model = { provider: "openai-codex", id: "gpt-5.5" } as any;
  const registry = {
    find: (provider: string, id: string) => (provider === "openai-codex" && id === "gpt-5.5" ? model : undefined),
    getAvailable: () => [model],
    getAll: () => [model],
  } as any;

  const agent = new WorkflowAgent({ cwd: "/tmp", modelRegistry: registry });
  const resolved = (agent as any).resolveModelAndThinking("openai-codex/gpt-5.5:high");
  assert.equal(resolved.model, model);
  assert.equal(resolved.thinkingLevel, "high");
});

test("WorkflowAgent.resolveModelAndThinking preserves exact model IDs containing non-thinking colons", () => {
  const model = { provider: "openrouter", id: "some/model:exacto" } as any;
  const registry = {
    find: (provider: string, id: string) => (provider === "openrouter" && id === "some/model:exacto" ? model : undefined),
    getAvailable: () => [model],
    getAll: () => [model],
  } as any;

  const agent = new WorkflowAgent({ cwd: "/tmp", modelRegistry: registry });
  const resolved = (agent as any).resolveModelAndThinking("openrouter/some/model:exacto");
  assert.equal(resolved.model, model);
