...
import { readWorktreeMarker, removeWorktreeMarker, saveWorktreeMarker, } from "./lib/worktree-shared.js";
...
export default function worktreeExtension(pi) {
    ...
    async function canResumeWorktree(info, repoRoot) {
        if (info.repoRoot !== repoRoot || !fs.existsSync(info.path))
            return false;
        ...
        return (branchResult.code === 0 && branchResult.stdout.trim() === info.branch);
    }
    async function ensureLocalPiIgnored(repoRoot) {
        const excludeResult = await git(["rev-parse", "--git-path", "info/exclude"], repoRoot);
    ...
    function redirectCommandPaths(command) {
        if (!worktreeState)
            return command;
        return command.replace(worktreeState.repoRootPattern, worktreeState.info.path);
    }
    function findRepoPathOutsideCurrentWorktree(command) {
        if (!worktreeState)
            return null;
        const { info } = worktreeState;
        const repoPathPattern = new RegExp(`${escapeRegExp(info.repoRoot)}(?:/[^\\s'"\\\`;|&<>)]*)?`, "g");
        let match;
    ...
    function rewriteBashCommand(command) {
        ...
        if (hasParentTraversal(redirected)) {
            return {
                block: "Blocked bash command: parent-directory traversal is not allowed while --worktree is active",
            };
        }
    ...
    function createWorktreeBashOperations(info) {
    ...
    function activateWorktree(info) {
        worktreeState = {
            info,
            repoRootPattern: new RegExp(`${escapeRegExp(info.repoRoot)}(?=$|/)(?!/\\.pi/worktrees(?:/|$))`, "g"),
    ...
    async function confirmInTerminal(branch, wtPath, isDirty) {
        ...
        try {
            const dirtyWarning = isDirty
                ...
                : "";
            const answer = await rl.question(`${dirtyWarning}Delete worktree?\n"${branch}" at ${wtPath}\nRemove it? [Y/n] `);
    ...
    function redirectPath(filePath, options) {
        if (!worktreeState)
        ...
        if (path.isAbsolute(expanded) &&
            ...
            if (isWithinDirectory(expanded, path.join(info.repoRoot, ".pi", "worktrees"))) {
                return {
    ...
    function redirectToolPath(event) {
        if (!worktreeState)
            return undefined;
        if (isToolCallEventType("bash", event)) {
            ...
            if ("block" in rewritten)
                return blockReason(rewritten.block);
        ...
        if (isToolCallEventType("read", event) ||
            ...
            if (typeof event.input.path === "string") {
                ...
                if ("block" in redirected)
                    return blockReason(redirected.block);
    ...
    pi.on("session_start", async (event, ctx) => {
        ...
        const branchChecks = await Promise.all(["main", "master"].map(async (branch) => ({
            branch,
            exists: (await git(["rev-parse", "--verify", `refs/heads/${branch}`], repoRoot)).code === 0,
        })));
        ...
        const wtDir = path.join(repoRoot, ".pi", "worktrees", `pi-wt-${worktreeId}`);
        const res = await git(["worktree", "add", "-b", branch, wtDir, base], repoRoot);
        if (res.code !== 0) {
    ...
    pi.on("session_shutdown", async (event, ctx) => {
        if (!worktreeState || event.reason !== "quit")
            return;
        const { path: wtPath, branch, repoRoot } = worktreeState.info;
        const statusResult = await git(["status", "--porcelain", "--untracked-files=normal"], wtPath);
        if (statusResult.code !== 0) {
            terminalMessage(`Could not inspect worktree status; keeping ${branch}: ${statusResult.stderr.trim() || statusResult.stdout.trim()}`);
            return;
        }
        ...
        if (shouldDelete) {
            ...
            if (removeResult.code !== 0) {
                terminalMessage(`Failed to delete worktree: ${removeResult.stderr.trim() || removeResult.stdout.trim()}`);
                return;
            }
